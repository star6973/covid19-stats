...
const Bump = () => {
    ...
    useEffect(() => {
        ...
        const createDataForm = items => {
            ...
            let refineAgeData = dataForm.filter(val => (val.gubun !== "여성" && val.gubun !== "남성"));
            let refineGenderData = dataForm.filter(val => (val.gubun === "여성" | val.gubun === "남성"));

            //! 연령별 확진률, 치명률, 사망률 그래프
            const ageList = ["0-9", "10-19", "20-29", "30-39", "40-49", "50-59", "60-69", "70-79", "80 이상"];
            const refineAgeConfirmedData = ageList.map(item => refineAgeData.filter(el => item === el.gubun)).map(data => {
                let temp = [];                
            ...
            const refineAgeCriticaldData = ageList.map(item => refineAgeData.filter(el => item === el.gubun)).map(data => {
                let temp = [];                
                for (let i = 0; i < data.length; i++) {
                    let x = data[i].formDate;
            ...
            const refineAgeDeathData = ageList.map(item => refineAgeData.filter(el => item === el.gubun)).map(data => {
                let temp = [];                
                for (let i = 0; i < data.length; i++) {
                    let x = data[i].formDate;
            ...
            //~ 성별 확진률, 치명률, 사망률 그래프
            const manConfirmedData = refineGenderData.filter(el => el.gubun === "남성").pop();
            const womanConfirmedData = refineGenderData.filter(el => el.gubun === "여성").pop();
